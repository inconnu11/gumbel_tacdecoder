...
class Utterances(data.Dataset):
    ...
    def __init__(self, root_dir, feat_dir, mode):
        ...
        print('Finished loading the {} Utterances {} dataset...'.format(self.num_tokens, mode))
    ...
    def load_data(self, submeta, dataset, idx_offset, mode):  
        for k, sbmt in enumerate(submeta):    
            ...
            uttrs[1] = sbmt[1]
            # fill in data
            for x in range(2,len(sbmt)):
                sp_tmp = np.load(os.path.join(self.root_dir, sbmt[x]))
                f0_tmp = np.load(os.path.join(self.feat_dir, sbmt[x]))
                if self.mode == 'train':
                    ...
                    raise ValueError
                uttrs[x] = ( sp_tmp, f0_tmp )
            dataset[idx_offset+k] = uttrs
           #如果按照先前错误的情况，dataset会是一个20个元素的数组。每个元素是：说话人名 one hot [mel1,f01] [mel2,f02].... 
                   
        
    def __getitem__(self, index):
        dataset = self.train_dataset if self.mode == 'train' else self.test_dataset
        #如果是上面错误的情况，则每次在这里get data我只会拿到20个说话人里的第一个数据 即please call。
        list_uttrs = dataset[index]
        spk_id_org = list_uttrs[0]
        emb_org = list_uttrs[1]
        
        melsp, f0_org = list_uttrs[2]# 问题出在这了
