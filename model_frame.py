...
class Decoder_att(nn.Module):
    ...
    def decode(self, decoder_input, spk):
        ...
        cell_input = torch.cat(
            (decoder_input, self.attention_context_1, self.attention_context_2, self.attention_context_3), -1)
        # 应该为343
        ...
        decoder_input = torch.cat(
            (self.attention_hidden, self.attention_context_1, self.attention_context_2, self.attention_context_3, spk),
            -1)
    ...
    def forward(self, decoder_inputs, memory_1, memory_lengths_1, memory_2, memory_lengths_2, memory_3,
                memory_lengths_3, spk):
    ...
    def inference(self, memory_1, memory_2, memory_3, spk):
        ...
        while True:
            ...
            if torch.sigmoid(gate_output.data) > self.gate_threshold:
                ...
                print("Warning! Reached max decoder steps", flush=True)
                break
...
class Parrot(nn.Module):
    ...
    def __init__(self, hparams, config):
        ...
        self.mask_part = self.mask_p.repeat(hparams.max_len_pad // 4 + 1, 1)[:hparams.max_len_pad,
                         :]  # padding to (408,c+r+f+e)
        ...
        self.ortho = TransformerSpecPredictionHead(hparams,
                                                   self.dim_neck * 2 + hparams.enc_mbv_size + self.dim_neck_3 * 2 + hparams.embedding_spk,
                                                   self.device)
    ...
    def forward(self, x_f0, x_org, c_trg, content_length, rhythm_length, pitch_length):
        ...
        code_exp_2 = code_exp_2.view(code_exp_2.size(0), code_exp_2.size(1), hparams.enc_mbv_size,
                                     2)  # batch,t_step,7,2
        # rhythm 上采样
        ...
        ortho_inputs_integral = torch.cat(
            (code_exp_1, code_exp_2, code_exp_3, speaker_embedding.unsqueeze(1).expand(-1, x_1.size(-1), -1)),
            dim=-1)  # 16,408,c+r+f+e 184
        ...
        feature_predict_C_P_U = torch.cat((feature_predict[:, :, :self.dim_neck * 2],
                                           feature_predict[:, :, self.dim_neck * 2 + hparams.enc_mbv_size:]), dim=-1)
        mask_part_C_P_U = self.mask_C_P_U
        invert_mask_C_P_U = self.invert_mask_C_P_U
        ortho_inputs_integral_C_P_U = torch.cat((ortho_inputs_integral[:, :, :self.dim_neck * 2],
                                                 ortho_inputs_integral[:, :,
                                                 self.dim_neck * 2 + hparams.enc_mbv_size:]), dim=-1)
        ...
        ortho_inputs_integral_R = ortho_inputs_integral[:, :,
                                  self.dim_neck * 2:self.dim_neck * 2 + hparams.enc_mbv_size]

        # outputs = [mel_outputs, feature_predict, ortho_inputs_integral, mask_part, invert_mask]
        return mel_outputs, [feature_predict_C_P_U, feature_predict_R], [ortho_inputs_integral_C_P_U,
                                                                         ortho_inputs_integral_R], [
                   mask_part_C_P_U, mask_part_R], [invert_mask_C_P_U, invert_mask_R], [alignments_1, alignments_2,
                                                                                       alignments_3]
